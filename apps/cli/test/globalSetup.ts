 ...
 * `cli.test.ts` reuses `apps/agent/test/helpers.ts` by relative import, and
 * that helper's `DATABASE_URL` reads `process.env.DATABASE_URL`, so setting
 * it here (before any test file loads) is sufficient — no helper changes.
 */
...
const PACKAGE_DB = "signalai_test_cli";
